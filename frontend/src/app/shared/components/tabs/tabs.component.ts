...
export class ZardTabComponent {
  ...
  // allow consumers to mark a tab as disabled (readonly view)
...
@Component({
        ...
        <nav [ngClass]="navClasses()" #tabNav role="tablist" [attr.aria-orientation]="horizontal ? 'horizontal' : 'vertical'">
          @for (tab of tabs(); track $index; let index = $index) {
            <button
              ...
              (click)="tab.disabled() ? null : setActiveTab(index)"
  ...
  styles: [
    `
      /* scrolling helper retained */
      .nav-tab-scroll {
        -webkit-overflow-scrolling: touch;
        scroll-behavior: smooth;
      }

      /* Pill-shaped tab bar background (light grey). The nav will hold the
         tab buttons which we force to equally distribute with flex:1 */
      z-tab-group nav[role="tablist"] {
        display: flex;
        gap: 0.5rem;
        background-color: #eef1f3; /* light grey */
        padding: 4px;
        border-radius: 10px; /* pill */
        align-items: center;
      }

      /* Make each tab take equal width and center text. Remove default borders
         so we don't show an underline/line indicator. */
      z-tab-group nav[role="tablist"] button[role="tab"] {
      ...
      /* Selected tab becomes white (raised look) */
      z-tab-group nav[role="tablist"] button[role="tab"][aria-selected="true"] {
        background: #ffffff;
        color: #111827;
      }

      /* Disabled look */
      z-tab-group nav[role="tablist"] button[role="tab"][aria-disabled="true"] {
        opacity: 0.6;
        pointer-events: none;
      }

      /* Small scrollbar styling for horizontal overflow */
      z-tab-group nav[role="tablist"]::-webkit-scrollbar {
        height: 6px;
      }
      z-tab-group nav[role="tablist"]::-webkit-scrollbar-thumb {
